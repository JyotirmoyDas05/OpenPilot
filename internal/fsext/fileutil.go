...
type FastGlobWalker struct {
	...
	extraIgnore *ignore.GitIgnore // formerly .openpilotignore
	rootPath    string
}
...
func NewFastGlobWalker(searchPath string) *FastGlobWalker {
	...
	// Load .openpilotignore if it exists
	extraIgnorePath := filepath.Join(searchPath, ".openpilotignore")
	if _, err := os.Stat(extraIgnorePath); err == nil {
		if ci, err := ignore.CompileIgnoreFile(extraIgnorePath); err == nil {
			walker.extraIgnore = ci
		}
	}
...
// ShouldSkip checks if a path should be skipped based on gitignore, .openpilotignore, and hidden file rules
func (w *FastGlobWalker) ShouldSkip(path string) bool {
	...
	if w.extraIgnore != nil {
		if w.extraIgnore.MatchesPath(relPath) {
			return true
		}
	}

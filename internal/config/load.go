...
// PushPopOpenPilotEnv normalizes OPENPILOT_* variables (legacy CRUSH_* support removed).
func PushPopOpenPilotEnv() func() {
	found := []string{}
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "OPENPILOT_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			key := strings.TrimPrefix(pair[0], "OPENPILOT_")
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("OPENPILOT_"+ev))
	}
	return func() {
...
func globalConfig() string {
	...
	// return path to main config directory
...
func GlobalConfigData() string {
	...
	// return path to main data directory
	if runtime.GOOS == "windows" {
		localAppData := os.Getenv("LOCALAPPDATA")
		if localAppData == "" {
			localAppData = filepath.Join(os.Getenv("USERPROFILE"), "AppData", "Local")

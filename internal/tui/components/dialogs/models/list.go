...
func (m *ModelListComponent) SetModelType(modelType int) tea.Cmd {
	...
	for _, provider := range m.providers {
		...
		// Add a provider-level action to allow setting/changing the provider API key
		provItem := list.NewCompletionItem("Set API key...", ModelOption{
			Provider: provider,
			Model:    catwalk.Model{ID: "__provider__", Name: ""},
		},
			list.WithCompletionID(
				fmt.Sprintf("%s:%s", provider.ID, "__provider__"),
			),
		)
		group.Items = append(group.Items, provItem)

...
func (m *modelDialogCmp) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyPressMsg:
		...
		case key.Matches(msg, m.keyMap.Select):
			...
			// If the user selected the provider action item, open API key input for that provider
			if selectedItem.Model.ID == "__provider__" {
				m.needsAPIKey = true
			...
			// Check if provider is configured for the selected model

...
import (
	...
	"github.com/JyotirmoyDas05/openpilot/internal/tui/components/files"
	"github.com/JyotirmoyDas05/openpilot/internal/tui/components/logo"
	lspcomponent "github.com/JyotirmoyDas05/openpilot/internal/tui/components/lsp"
	"github.com/JyotirmoyDas05/openpilot/internal/tui/components/mcp"
...
func (m *sidebarCmp) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case chat.SessionClearedMsg:
		...
		// Regenerate logo when session is cleared
		m.logo = m.logoBlock()
	...
	case pubsub.Event[session.Session]:
		if msg.Type == pubsub.UpdatedEvent {
			if m.session.ID == msg.Payload.ID {
				...
				// Regenerate logo when session is updated
				m.logo = m.logoBlock()
			}
		}
	}
...
func (m *sidebarCmp) logoBlock() string {
	...
	// When a chat session is active, use the stacked OPEN/PILOT layout
	// for better branding visibility in the sidebar
	if m.session.ID != "" {
		return logo.RenderChat(version.Version, opts)
	}

	// Default to compact horizontal layout when no session is active
	return logo.Render(version.Version, true, opts)
}
...
func (m *sidebarCmp) calculateAvailableHeight() int {
	...
	if !m.compactMode {
		if m.height > LogoHeightBreakpoint {
			// Calculate actual logo height based on session state
			if m.session.ID != "" {
				// Stacked OPEN/PILOT layout: meta row (1) + OPEN (3) + separator (1) + PILOT (3) = 8 lines
				usedHeight += 8
			} else {
				// Compact horizontal layout is shorter
				usedHeight += 4
			}
		} else {
...
func (m *sidebarCmp) SetSession(session session.Session) tea.Cmd {
	m.session = session
	// Regenerate logo when session is set
	m.logo = m.logoBlock()

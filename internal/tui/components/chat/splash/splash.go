...
func (s *splashCmp) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case spinner.TickMsg:
		...
		return s, cmd
	case tea.MouseMsg:
		// re-render logo on mouse activity to animate the header
		s.logoRendered = s.logoBlock()
		return s, nil
	}
	return s, nil
...
func (s *splashCmp) View() string {
	...
	if s.needsAPIKey {
		...
		initText := lipgloss.JoinVertical(
			...
			bodyStyle.Render("result into an OPENPILOT.md file which serves as general context."),
			"",

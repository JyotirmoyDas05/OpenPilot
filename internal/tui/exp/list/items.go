...
import (
	...
	"github.com/JyotirmoyDas05/openpilot/internal/tui/components/core/layout"
...
func WithCompletionFocusedShortcut(shortcut string) CompletionItemOption {
	return func(cmp *options) {
		cmp.focusShortcut = shortcut
	}
}
...
func NewCompletionItem[T any](text string, value T, opts ...CompletionItemOption) CompletionItem[T] {
	...
	c.focusShortcut = o.focusShortcut
	return c
}
...
func (c *completionItemCmp[T]) View() string {
	...
	// We'll right-align the shortcut text to the far right and style it smaller/green when focused
	shortcut := ""
	if c.shortcut != "" || c.focusShortcut != "" {
		shortcut = c.shortcut
		if c.focus && c.focusShortcut != "" {
			shortcut = c.focusShortcut
		}
	}

	// Prepare styles
	smallStyle := t.S().Muted.Copy().Faint(true)
	focusedShortcutStyle := t.S().Base.Foreground(t.Green).Bold(false)
	if c.focus {
		smallStyle = smallStyle.Copy().Faint(true)
	}
	...
	if shortcut != "" {
		if c.focus {
			right = focusedShortcutStyle.Render(shortcut)
		} else {
			right = smallStyle.Render(shortcut)
		}
	}

	// If the focused shortcut looks like a change action (configured provider), render it on a second line
	if c.focus && shortcut != "" && (startsWithChange(shortcut) || startsWithChange(c.focusShortcut)) {
		// use same focused shortcut style as the single-line 'Set API key...' to increase visibility
		focusedLineStyle := t.S().Base.Foreground(t.Green).Bold(true)
		secondLine := focusedLineStyle.Render(shortcut)
		content := lipgloss.JoinVertical(lipgloss.Left, left, secondLine)
		return itemStyle.Render(content)
	}

	// Join left and right with flexible spacing so right is pushed to far right.
	row := lipgloss.JoinHorizontal(lipgloss.Left, left, lipgloss.NewStyle().Width(innerWidth-lipgloss.Width(left)).Align(lipgloss.Right).Render(right))
	item := itemStyle.Render(row)
	return item
}
...
func (c *completionItemCmp[T]) GetSize() (int, int) {
	// If the focused shortcut is a change action, height is 2 lines when focused
	if c.focus && c.focusShortcut != "" && (startsWithChange(c.focusShortcut) || startsWithChange(c.shortcut)) {
		return c.width, 2
	}
	return c.width, 1
}
...
func startsWithChange(s string) bool {
	return strings.HasPrefix(s, "Change")
}

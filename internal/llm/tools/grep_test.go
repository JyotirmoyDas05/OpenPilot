...
func TestGrepWithIgnoreFiles(t *testing.T) {
	...
	// Create .openpilotignore file
	crushignoreContent := "node_modules/\n"
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".openpilotignore"), []byte(crushignoreContent), 0o644))
	...
	// node_modules/lib.js should be ignored by .openpilotignore
...
func TestSearchImplementations(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".openpilotignore"), []byte("file5.txt\n"), 0o644))

...
import (
	...
	"github.com/JyotirmoyDas05/openpilot/internal/llm/prompt"
	"github.com/JyotirmoyDas05/openpilot/internal/llm/provider"
	"github.com/JyotirmoyDas05/openpilot/internal/llm/tools"
	"github.com/JyotirmoyDas05/openpilot/internal/log"
	"github.com/JyotirmoyDas05/openpilot/internal/lsp"
	...
	"github.com/JyotirmoyDas05/openpilot/internal/shell"
	...
	for event := range eventChan {
		if processErr := a.processEvent(ctx, sessionID, &assistantMsg, event); processErr != nil {
			if errors.Is(processErr, context.Canceled) {
				...
				a.finishMessage(ctx, &assistantMsg, message.FinishReasonError, "API Error", providerErrorDetails(processErr))
			}
...
// providerErrorDetails inspects an error returned by a provider and returns
// a possibly augmented details string with actionable guidance for known
// provider-specific errors (for example OpenRouter policy blocks).
func providerErrorDetails(err error) string {
	if err == nil {
		return ""
	}
	s := err.Error()
	// Detect OpenRouter policy errors and surface the settings link
	lower := strings.ToLower(s)
	if strings.Contains(lower, "openrouter") || strings.Contains(s, "Enable prompt training") || strings.Contains(s, "openrouter.ai/settings/privacy") {
		// Append a short, actionable hint for users
		s = s + "\n\nOpenRouter account policy appears to be blocking this request. Enable prompt training or update privacy settings here: https://openrouter.ai/settings/privacy"
	}
	return s
}
